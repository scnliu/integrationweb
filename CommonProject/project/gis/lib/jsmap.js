...
function jsmCreatePushPin(rcdNdx, dsNdx, ppNdx, evRcd)
{
	...
	//alert(rcdNdx+":"+dsNdx+":"+ppNdx+":"+evRcd+":"+lat+":"+lon+":"+dev+":"+html+":"+pp.iconURL+":"+pp.iconSize+":"+pp.iconOffset+":"+pp.shadow+":"+pp.shadowSize);
...
function jsmHighlightDetailRow(rcdNdx, highlight)
{
    ...
    if ((rowNdx >= 1) && highlight) {
		...
		/* syn here... */
		try{
			var h = jsvDetailPoints[rowNdx-1].speed;			
			drawOne(210+rowNdx*4,650-h,4,h,"red",rcdNdx,highlight);
		}catch(e){
...
// ----------------------------------------------------------------------------
			...
			function _mkDiv(x, y, w, h, color){    
				idiv = '<div onclick="" style="position:absolute;'+'left:' + x + 'px;'+'top:' + y + 'px;'+'width:' + w + 'px;'+'height:' + h + 'px;'+'clip:rect(0px,'+w+'px,'+h+'px,0px);'+'background-color:'+color+';"><\/div>';    
				return idiv;   
			} 
			function drawOne(x,y,w,h,color,rcdNdx,highlight){		
				var c = "red";
				if(h>20){
					c = "green";
				}else if(h>5){
				...
				document.getElementById(trackMap).innerHTML += _mkDiv(x,y,w,h,c);
			}
			function test(){				
				for(var i = 0; i<500; i++){
					var x = 210+i;
