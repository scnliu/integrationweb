    ...
    $.ligerAddGrid = function(grid, p)
    {
        ...
        var g = {
            ...
            deleteServerData:function(){
	            ...
	            askFunc('确定删除所选记录!',g.deleteServerData_);
            },
            deleteServerData_:function(){
	            var rows = g.getCheckedRows();
	            if(rows.length==0){
	            	notify('请选择记录!');
	            	return;
            ...
            page:function(ele,type){
		        if(!($(ele).hasClass('page-first-disabled')||$(ele).hasClass('page-prev-disabled')
				        ||$(ele).hasClass('page-next-disabled')||$(ele).hasClass('page-last-disabled'))){
            ...
            buildPagerBar:function(){
            
            	 $('#useCurrentPage').html(p.page);
            	 $('#usePageTotal').html(p.pageCount);
                 ...
                 $('#useRowTotal').html(p.total);
                 if(p.page==1)
                 {
                      $(".paging .page-first").addClass("page-first-disabled");
                      $(".paging .page-prev").addClass("page-prev-disabled");
//                      $(".paging .page-first").removeClass("page-first");
//                      $(".paging .page-prev").removeClass("page-prev");
                 }
                 else
                 {
//                	 $(".paging .page-first-disabled").addClass("page-first");
//                     $(".paging .page-prev-disabled").addClass("page-prev");
                	 $(".paging .page-first-disabled").removeClass("page-first-disabled");
                     $(".paging .page-prev-disabled").removeClass("page-prev-disabled");
                 }
                 if(p.page==p.pageCount)
                 {
                     $(".paging .page-last").addClass("page-last-disabled");
                      $(".paging .page-next").addClass("page-next-disabled");
//                      $(".paging .page-last").removeClass("page-last");
//                      $(".paging .page-next").removeClass("page-next");
                 }
                 else
                 {
//                	  $(".paging .page-last-disabled").addClass("page-last");
//                      $(".paging .page-next-disabled").addClass("page-next");
                     $(".paging .page-last-disabled").removeClass("page-last-disabled");
                     $(".paging .page-next-disabled").removeClass("page-next-disabled");
                 }
            },
            buildPager: function()
            {
       ...
       $.fn.ligerCheckBox &&  $('input:checkbox', g.popup).ligerCheckBox(
        ...
        grid.usedGrid = true;
        //指定页数
        $('#'+grid.id+'_pageBar .to_page').keyup(function(evt){
        	if(evt.keyCode==13){
           		$(this).trigger('toPage');
            }
        }).bind('toPage',function(){
         	var value=$(this).val();
        	if(/^([1-9]\d*)$/.test(value)){
        	 if (value>=p.pageCount){
        	 ...
        	 $(this).val(p.newPage);
//              g.setOptions({newPage:value}); 
              g.loadData(); 
	        }else{
	        	$(this).val('');
		     }
        });
    $('#'+grid.id+'_pageBar .to_page_go').click(function(evt){
    	 $('#'+grid.id+'_pageBar .to_page').trigger('toPage');
    	 return false;
        });
