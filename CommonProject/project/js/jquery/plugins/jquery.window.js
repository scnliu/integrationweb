...
$.Window = (function()  {
	...
	var setting = {
		...
		dockArea: "bottomBarM",                 // [jquery object, element:null] the area which the windows will dock at
	...
	// push the window instance into minimized storage//可以去掉
	...
	function constructor(caller, options) {
		...
		var options = $.extend({
			...
			isMax:false,                              // or you can set it as null to hide icon.
			...
			z: 20,                        // [number:2000] the css z-index value
		...
		var windowId = "window_" + options.id; // the window's id by hezheng
		...
		function initialize(instance) {
			...
			// add close button
			if( options.closable ) {
				headerFuncPanel.append( "<div title='关闭' class='closeImg window_icon_button no-draggable'></div>" );
			...
			// add maximize button
			if( options.maximizable ) {
				headerFuncPanel.append( "<div title='最大化' class='maximizeImg window_icon_button no-draggable'></div>" );
				headerFuncPanel.append( "<div title='还原' class='cascadeImg window_icon_button no-draggable' style='display:none;'></div>" );
			...
			if( options.minimizable ) {
				headerFuncPanel.append( "<div title='最小化' class='minimizeImg window_icon_button no-draggable'></div>" );
...
//				headerFuncPanel.append( "<div title='添加到收藏夹' class='bookmarkImg window_icon_button no-draggable'></div>" );
			...
			if( options.url != null && $.trim(options.url) != "" ) {
				...
				container.append("<iframe name='iframe_'"+options.id+"' id='iframe_'"+options.id+"' style='display:none;' APPLICATION='yes' class='window_frame ui-widget-content no-draggable no-resizable "+options.frameClass+"' scrolling='"+scrollingHtml+"' src='"+options.url+"' width='100%' height='"+frameHeight+"px' frameborder='0'></iframe>");
...
//						hideContent();
		...
		function setBottomBar(){
			var bottomDiv=$('#'+setting.dockArea);
			bottomBar=$("<div class='bottomBarMDiv deskBarbg'></div>");
			bottomBar.attr({"id":"bottom_"+options.id});
		    
		
			var Href=$('<a></a>');
			    Href.css("background-image","url("+options.bottomIcon+")");    
			    Href.addClass("span1");
			    
			var Span=$('<span></span>');
			    Span.html(options.title);
			    Span.addClass("span2");
			    
			   bottomBar.append(Href);
			   bottomBar.append(Span);
		       
			 //右键菜单
			   var  menu = $.ligerMenu({ top: 0, 
				   left: 20, width: 135, items:
		            [
		             { text: '关闭当前窗口', click: close},
		             { line: true },
		             { text: '关闭所有窗口', click: closeAll},
		             { line: true },
		             { text: '刷新当前窗口', click: refreshWin }
		             ]
		             });
			   
			   
			 bottomBar.bind('click',function(){
				if(caller.get(0)._lastSelectedWindow!= null &&
						caller.get(0)._lastSelectedWindow == _this){
                ...
                menu.show({ top: e.pageY-95, left: e.pageX-50 });
                return false;
            });

			 bottomDiv.append(bottomBar);
		}
...
//			// must add this, or it will get a bug when user mouse down the border of window panel if it is resized.
			...
			if( caller != null ) {
				...
				targetCssStyle = {
					...
					top: cpos.top,
					width: caller.width(),
					height: caller.height()-$('#'+setting.dockArea).height()
//					,
//					opacity: 1
				};
			} else {
...
//				opacity: 1
	...
	return { // static public methods
		getInstance: function(caller, options) { // create new window instance
			...
			//判断如果已经创建就返回空  by hezheng
			//存在的窗口显示，其它窗口隐藏.hhs
			var existsWindow=false;
			 for( var i=0, len=windowStorage.length; i<len; i++ ) {
			     var wId=windowStorage[i].getWindowId();
			     if(instance.getWindowId()==wId){
			    	 windowStorage[i].show();
			    	 existsWindow=true;
			    	 break;
			     }
		     }
