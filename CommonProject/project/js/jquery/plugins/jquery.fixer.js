		...
		return $(this)
				.each(
						function() {
							...
							//设置最外层的div左偏移
							var divMarginLeft=opts.divMarginLeft;
							...
							if (fixrows) {
									...
									//colgroup.append($("<col></col>").css("width",sbwidth));
							...
							var wrapper = $("<div></div>");
							wrapper.css('margin-left',divMarginLeft);
							wrapper.css('border','1px solid #B2EAFB');
							wrapper.css('border-top','0px solid #B2EAFB');
							...
							if (fixrows) {
								...
								//放topright层到topright1层中(update)
								var topright1=$("<div></div>");
								topright1.addClass('head');
								
							    topright.css("overflow", "hidden").css('margin', '0').css('padding', '0');
							
								//topright.css("margin-right","5px");
								if ($.browser.msie){
									//topright.css("float", "left");
									topright1.css("float", "left");
								}
								topright.append(toprighttable);
								
								//放topright1到topright层中(update)
								topright1.append(topright);
								
								//update
								wrapper.append(topright1);
						...
						//	bottomright.css("margin-left","5px");
							if (fixcols)
							...
							if (setwidth) {
								...
								if (fixrows){
								topright1.css("width", width-fixedwidth);
