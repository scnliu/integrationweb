...
(function($){	
	...
	bValidator = function(mainElement, overrideOptions){
		...
		var options = {
			...
			offset:              {x:-25, y:18},	// offset position for error message tooltip
			position:            {x:'right', y:'top'}, // error message placement x:left|center|right  y:top|center|bottom
			template:            '<div class="{errMsgClass}" onmouseover="$(this).fadeOut(\'normal\');" onclick="$(this).css(\'visibility\', \'hidden\');">'+
			'<div class="arrow"></div><div class="msgcontent">{message}</div></div>', // template for error message
		...
		//		messagesHtml = options.templateCloseIcon.replace('{message}', messagesHtml).replace('{closeIconClass}', options.closeIconClass).replace('{closeErrMsg}', '$(this).closest(\'.'+ options.errMsgClass +'\').css(\'visibility\', \'hidden\');');
			...
			tooltip.css('width',messagesHtml.length*17);
			tooltip.css({visibility: 'visible', position: 'absolute', top: pos.top, left: pos.left}).fadeTo(options.showErrMsgSpeed,0.9);
			...
			msg_container.click(function(){element.focus();});
			element.focus(function(){$(this).data('errMsg.bV').hide();});
		},
		...
		_getErrMsgPosition = function(input2, tooltip){

		        var tooltipContainer = input2.data("errMsg.bV");
		        var input =input2,
		...
		if(mainElement.is('form')){
			...
			mainElement.bind('keyup', function(event){
				if(instance.validate())
					return true;
				else{
					event.stopImmediatePropagation();
				...
				if(!is_valid){
					...
					for(var i in actions){
						...
						if(typeof validator[validatorName] == 'function'){
							...
							if(validatorName!=null&&validatorName=='checkCode'){
								validatorParams[0]=$(this);
							}
