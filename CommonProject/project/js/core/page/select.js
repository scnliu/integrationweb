...
function O(div) {
	var $div=$(div);
	this.id = div.id;
	this.x = $div.css('left').replace('px','');
	this.y = $div.css('top').replace('px','');
...
function startDrag(e,mainId,divArray) {
	...
	$(mainId).bind("mousemove",drag); 
		if(!move) {
			var dragDiv = dojo.byId(curDiv+"_dragging"); 
			$(dragDiv).css('z-index',2000);
			x = e.clientX;
...
function drag(e) {
	if(move) {
		...
		var $dragDiv=$(dragDiv);
		if(e.clientX- x >= 0) {
			$dragDiv.width(e.clientX+$(document).scrollLeft()- x);
			$dragDiv.css('left',x);
		} else {
			$dragDiv.width(x-e.clientX);
			$dragDiv.css('left',e.clientX+$(document).scrollLeft());
		}
		if(e.clientY+$(document).scrollTop()-y>= 0) {
			$dragDiv.height(e.clientY+$(document).scrollTop()-y);
//			dragDiv.style.top = y;
			$dragDiv.css('top',y);
		} else {
			$dragDiv.height(y - e.clientY-$(document).scrollTop());
			$dragDiv.css('top',e.clientY+$(document).scrollTop());
		}
